...
| `peft-mora/` | فرع PEFT المحلي الذي يضيف MoRA وGL-log-MoRA، بما في ذلك Q ومنتظم log-determinant والمسارات الكمية. |
| `train_legacy_deepspeed.py` | نسخة احتياطية من نقطة الدخول القديمة الخاصة بـ LLaMA/DeepSpeed، ولا تُستخدم في المسار الجديد. |
| `colab_train_qwen25_3b_mora.ipynb` | دفتر Colab ينفذ الاستنساخ، التثبيت، تنزيل البيانات، الفحص، smoke test، التدريب، الاستئناف، إعادة التحميل، والدردشة مع الـadapter، مع حفظ محلي احتياطي عند تعذر Drive. |
| `chat_with_adapter.py` | مشغل طرفية للدردشة مع Qwen والـadapter المحفوظ بعد التدريب. |
| `RUNBOOK.md` | دليل التشغيل الكامل عبر Colab أو جهاز محلي، مع أوامر التدريب والاختبار والدردشة. |
...
عند تفعيل `use_gl_log_mora=true` مع `use_mora=true`، تُنشئ كل طبقة MoRA معامل `Q` مهيأً بالهوية، وتطبق التحويل داخل فضاء الرتبة، وتضيف إلى الخسارة متوسط المنتظم:

```text
-λ · log det(QᵀQ + δI)
```

يستخدم `GLLogMoRATrainer` معدل تعلم مستقلًا لـQ عبر `gl_log_q_lr`، بينما يبقى معدل تعلم معاملات MoRA الأساسية في `learning_rate`. القيم الافتراضية هي `λ=0.01` و`δ=1e-3` و`gl_log_q_lr=3e-4`. هذه نسخة إعادة تنفيذ من الورقة وليست كود المؤلف الرسمي.

لتفعيلها من سطر الأوامر:

```bash
python train.py --config configs/qwen25_3b_mora.json --use_gl_log_mora
```
...
| learning rate | `2e-4` | معدل تعلم معاملات MoRA الأساسية. |
| use_gl_log_mora | `true` في إعداد GL المقترح | يفعّل Q ومنتظم log-determinant. |
| gl_log_lambda | `0.01` | وزن المنتظم. |
| gl_log_delta | `1e-3` | مثبت عددي موجب داخل log-det. |
| gl_log_q_lr | `3e-4` | معدل تعلم Q المنفصل. |
...
عند `use_mora=true` يفحص السكربت وجود الحقل `use_mora` داخل `LoraConfig`. وعند `use_gl_log_mora=true` يتطلب الحقول الإضافية ويضيف منتظم log-det إلى loss ويفصل معدل تعلم Q. إذا كان المستورد يشير إلى PEFT العادي، يتوقف برسالة واضحة بدلاً من تدريب LoRA عادي وتسميته خطأً MoRA أو GL-log-MoRA. حزمة `peft-mora/` هنا هي إعادة تنفيذ محلية موثقة، وليست كودًا رسميًا منشورًا من مؤلفي GL-log-MoRA [2].
